...
import type { EnrollmentType } from "@/entities/enrollment";
    ...
    <div className="space-y-6">
      ...
      {error && (
        <Alert variant="destructive">
          ...
          <AlertDescription>강의 목록을 불러오는데 실패했습니다.</AlertDescription>
        </Alert>
      )}
      ...
      {validationErrors.courseId && (
        <Alert variant="destructive">
          <AlertCircle className="h-4 w-4" />
          <AlertDescription>{validationErrors.courseId}</AlertDescription>
        </Alert>
      )}
      ...
      <div className="space-y-3">
        ...
        <RadioGroup
          value={selectedType}
          onValueChange={(v) => setSelectedType(v as EnrollmentType)}
      ...
      <div className="flex justify-end">
        <Button onClick={handleNext} size="lg" disabled={!selectedCourseId || isLoading}>
          다음 단계
        </Button>
      </div>
    </div>
